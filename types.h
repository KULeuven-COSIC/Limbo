...
typedef std::array<uint8_t, SALT_SIZE> limbo_salt_t;

typedef std::pair<std::vector<std::vector<uint8_t>>, size_t> reveal_list_t;

typedef std::array<uint8_t, 16> aes_block_t;

typedef std::pair<std::vector<uint8_t>, std::vector<uint8_t>> limbo_keypair_t;

struct limbo_repetition_proof_t {
  reveal_list_t reveallist;
  std::vector<uint8_t> missing_seed_commitments;
  std::vector<uint8_t> witness_deltas;
  std::vector<uint8_t> mult_deltas;
  std::vector<std::vector<field::GF2E>> h_deltas;
  std::vector<field::GF2E> f_final;
  std::vector<field::GF2E> g_final;
  field::GF2E h_final;
};

struct limbo_proof_t {
  limbo_salt_t salt;
  std::vector<uint8_t> h_1;
  std::vector<std::vector<uint8_t>> compression_commitments;
