...
uint64_t reduce_GF2_64(__m128i in) {
  ...
  uint64_t R_upper = _mm_extract_epi64(in,1);
      //((_mm_extract_epi64(in, 1) & upper_mask) << 16) | (R_lower >> 48);
  uint64_t T = R_upper;
  R_upper = R_upper ^ (T >> 60) ^ (T >> 61) ^(T >> 63);
  R_lower = R_lower ^ (R_upper << 4) ^ (R_upper << 3) ^
            (R_upper << 1) ^ (R_upper << 0);
  return R_lower;
}
...
class int65
{
  ...
  int65() : lower(0), msb(false) {}
  int65(uint64_t lower, bool msb) : lower(lower), msb(msb) {}
  int65(uint64_t a) : lower(a), msb(false) {}
  //int65(word a)
  //{
  //  *this= a;
  //}
  uint64_t get_lower() { return lower; }
  int65 &operator=(const uint64_t &other)
  {
    lower= other;
    msb= false;
    return *this;
  }
  bool get_msb() {return msb;}
  //int65 &operator=(const word &other)
  //{
  //  lower= other;
  //  msb= false;
  //  return *this;
  //}
  bool operator==(const int65 &other)
  {
  ...
  int65 operator<<(int other)
  {
    return int65(lower << other, ((lower >> (64 - other)) & 1));
  }
  int65 &operator>>=(int other)
  {
    lower>>= other;
    lower|= (uint64_t(msb) << (64 - other));
    msb= !other;
    return *this;
  }
  int65 operator^(const int65 &other)
  {
    return int65(lower ^ other.lower, msb ^ other.msb);
  }
  int65 &operator^=(const int65 &other)
  {
    lower^= other.lower;
  ...
  int65 operator&(const int &other)
  {
    return int65(lower & other, false);
  }
...
uint64_t mod_inverse(uint64_t a, uint64_t mod, bool msb) {
  if (msb)
  {
    if (a == 1)
    {
...
void GF2E::init_extension_field(const limbo_instance_t &instance) {
  ...
  case 8: {
    // modulus = x^64 + x^4 + x^3 + 1
    modulus =
   (1ULL << 4) | (1ULL << 3) | (1ULL << 1) | (1ULL << 0);
    msb = true;
    reduce = reduce_GF2_64;
    byte_size = 8;
    // Ring morphism:
    //   From: Finite Field in x of size 2^8
    //   To:   Finite Field in y of size 2^64
    //   Defn: x |--> y^57 + y^56 + y^53 + y^51 + y^50 + y^47 + y^46 + y^45 +
    //   y^43 + y^39 + y^37 + y^36 + y^35 + y^34 + y^33 + y^31 + y^30 + y^28 +
    //   y^27 + y^26 + y^24 + y^23 + y^22 + y^19 + y^15 + y^13 + y^10 + y^9 +
    //   y^6 + y^4 + y^2 + y
    GF2E gen;
    gen.set_coeff(57);
    gen.set_coeff(56);
...
//returns the polynomial in the coefficient domain
...
//build the polynomial in the coefficient domain from the specified roots
